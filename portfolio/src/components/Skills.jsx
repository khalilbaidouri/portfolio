...
const Skills = () => {
  return (
    <section
      id="skills"
      className="py-20 min-h-screen text-gray-900 overflow-hidden relative"
    >
      <div
      ...
      <div className="relative w-full overflow-hidden group">
        <div className="flex animate-scroll gap-8 px-4 w-max group-hover:[animation-play-state:paused] cursor-pointer">
          {skillsData.concat(skillsData).map((skill, index) => (
            <div
              key={index}
              className="flex flex-col items-center justify-center border border-gray-300 rounded-2xl p-4 bg-white hover:bg-gray-100 transition-all duration-300 min-w-[120px] shadow-md hover:shadow-lg"
            >
              <img
                src={skill.icon}
                alt={skill.title}
